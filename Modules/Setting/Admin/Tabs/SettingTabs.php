...
class SettingTabs extends CiTabs
{
    ...
    public function make()
    {
        $this->group('general_settings', clean(trans('setting::settings.tabs.group.general_settings')))
            ...
            ->add($this->newsletter())
             // ->add($this->rave())
            // ->add($this->paystack())
            ->add($this->paymentSetup())
            ->add($this->storageSetup())
    ...
    private function rave()
    {
        return tap(new CiTab('rave', 'Flutterwave Settings'), function (CiTab $tab) {
            $tab->weight(20);
            ...
            $tab->view('setting::admin.settings.tabs.rave');
        });
    }
    ...
    {
        return tap(new CiTab('paystack', 'Paystack Settings'), function (CiTab $tab) {
            $tab->weight(20);
    ...
    private function paymentSetup()
    {
        return tap(new CiTab('payment', 'Default Aggregator'), function (CiTab $tab) {
            $tab->weight(20);
            $tab->fields([
                'payment_aggregator',   
            ]);
            $tab->view('setting::admin.settings.tabs.payment');
        });
    }


    private function storageSetup()
    {
        return tap(new CiTab('storage', 'Storage Location'), function (CiTab $tab) {
            $tab->weight(20);
            $tab->fields([
                'storage_location',   
            ]);
            $tab->view('setting::admin.settings.tabs.storage');
        });
    }
