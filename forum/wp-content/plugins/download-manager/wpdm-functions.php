    ...
    if (wpdm_query_var('action') === 'wpdmfilelistcd') {
        if (\WPDM\Session::get('wpdmfilelistcd_' . wpdm_query_var('pid', 'int'))) {
            ...
            if ($idvdl && ($pwdlock || !$olock)) {
                $dlcol = "<th align=center>" . __("Download", "download-manager") . "</th>";
                $swl = 1;
            }
            ...
            if (is_array($dfiles)) {
                ...
                foreach ($dfiles as $ind => $sfile) {
                    ...
                    if (is_dir($sfile)) {
                        $ext = 'folder';
                        $info = count(scandir($sfile)) . " " . __("files", "download-manager");
                    }
...
function wpdm_ask_for_custom_data($pid)
{
    ...
    <div class="form-group">
        <label><input type="checkbox" name="file[custom_form_field][name]"
                      value="1" <?php if (isset($cff['name']) && $cff['name'] == 1) echo 'checked=checked'; ?> > <?php _e("Ask for Visitor's Name", "download-manager"); ?>
        </label> <br/>
    </div>
