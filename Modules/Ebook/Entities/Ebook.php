...
class Ebook extends Model
{
    ...
    public function scopeWithMainBookPdf($query)
    {
        $query->with(['files' => function ($q) {
            $q->wherePivot('zone', 'main_book_file');
        }]);
    }
    ...
    {
        $query->with(['files' => function ($q) {
            $q->wherePivotIn('zone', [ 'book_cover','book_file', 'main_book_file']);
        }]);
    }
    ...
    // airon - ebook has many purchases
    public function purchases()
    {
        return $this->hasMany(PurchasedEbook::class, 'ebook_id');
    }
    ...
    {   
        ...
        $ebookPriceCurrency = Currency::find($this->currency_id);
        if(is_null($ebookPriceCurrency)){
            return;
        }
        return $ebookPriceCurrency->code;
    ...
    // airon here
    /**
     * Get ebook's Main Book PDF.
     *
     * @return \Modules\Media\Entities\File
     */
    public function getMainBookFileAttribute()
    {
       if($this->main_file_type == 'upload'){
            $file = $this->files->where('pivot.zone', 'main_book_file')->first() ?: new Files;
            return $file;
       }elseif ($this->main_file_type == 'external_link') {
    ...
    public function hasMainBookFile()
    {
        if ($this->files->where('pivot.zone', 'main_book_file')->first() || $this->main_file_url) {
            return true;
        }
        return false;
    }
     ...
     public function userHasPurchased()
    {
        ...
        if($customer){
            ...
            $currentPurchase = $customerPurchases->where('ebook_id', $this->id )->first();
            if($currentPurchase){
                return true;
            }else{
                return false;
