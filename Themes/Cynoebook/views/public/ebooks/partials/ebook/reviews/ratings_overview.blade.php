<div class="col-lg-12 col-md-5">
    ...
    <div class="review-form clearfix">
        <form method="POST" action="{{ route('ebooks.reviews.store', $ebook) }}" class="clearfix bg-w-form contact-form">
            {{ csrf_field() }}

           
            <div class="news-comment-title underline">{{ clean(trans('cynoebook::ebook.reviews.add_a_review')) }}</div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group {{ $errors->has('reviewer_name') ? 'has-error' : '' }}">
                        ...
                        <input type="text" name="reviewer_name" class="form-control form-input" id="reviewer-name" value="{{ old('reviewer_name', auth()->user()->full_name ?? '') }}">
                ...
                <div class="col-md-12">
                    <div class="form-group  {{ $errors->has('comment') ? 'has-error' : '' }}">
                        ...
                        <textarea name="comment" class="form-control form-input" id="comment" cols="30" rows="5">{{ old('comment') }}</textarea>
                ...
                <button type="submit" class="btn btn-primary btn-green review-submit" data-loading>
                    <span>{{ clean(trans('cynoebook::ebook.reviews.add_review')) }}</span>
                </button>
            </div>
        </form>
    </div>
